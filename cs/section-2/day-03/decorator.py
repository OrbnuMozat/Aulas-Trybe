...
class CalculadoraDecorada:
    ...
    def converterNumero(self, numero):
        ...
        # dicionário, conseguimos acessar e obter o valor a partir da chave
        return {
            "um": 1,
            "dois": 2,
            "três": 3,
...
class CalculadoraTradutoraDecorada:
    def __init__(self, calculadora) -> None:
        self.calculadora = calculadora

    def convertNumero(self, numero):
        if not isinstance(numero, str):
            return numero
        return {
            "one": 1,
    ...
    def soma(self, x, y):
        return self.calculadora.soma(
            self.convertNumero(x), self.convertNumero(y)
        )
...
if __name__ == "__main__":
    ...
    calculadoraTradDecorada = CalculadoraTradutoraDecorada(calculadora)
    print("two + five =", calculadoraTradDecorada.soma("two", "five"))
